 ...
 * a real particle system tuned for the Seamless Gutters 4 Less brand: cold
  ...
  useEffect(() => {
    ...
    const tick = (now) => {
      ...
      for (let i = 0; i < drops.length; i++) {
        ...
        if (d.copper > 0.2) {
          ...
          grad.addColorStop(1, "oklch(0.92 0.015 250 / 0)");
        } else {
          /* Far/mid drops: cool near-white, no warmth. */
          grad.addColorStop(0, `oklch(0.93 0.02 250 / ${d.a.toFixed(3)})`);
          grad.addColorStop(1, "oklch(0.92 0.02 250 / 0)");
        }
    ...
    const renderStill = () => {
      ...
      stateRef.current.drops.forEach((d) => {
        ...
        const y2 = d.y - d.len;
        ctx.strokeStyle = `oklch(0.92 0.02 250 / ${(d.a * 0.55).toFixed(3)})`;
        ctx.lineWidth = d.w;
        ctx.lineCap = "round";
        ctx.beginPath();
        ctx.moveTo(d.x, d.y);
