...
export default function TrustStack() {
  const yrs      = SITE.yearsDisplay();
  const counties = SITE.countiesServed.length;
  const installs = SITE.installsCount ?? "1,000+";
  const rating   = SITE.rating.value.toFixed(1);

  const stats = [
    { val: yrs,                  label: "Years on the ladder" },
    { val: installs,             label: "Puget Sound homes protected" },
    { val: rating, suffix: "★",  label: "Google rating" },
